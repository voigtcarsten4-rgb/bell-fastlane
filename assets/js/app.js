...
(function () {
  ...
  let backTarget = 'start';      // where "back" from an order detail goes
  ...
  function show(view) {
    ...
    if (node) { node.classList.remove('hide'); node.classList.add('anim-up'); setTimeout(() => node.classList.remove('anim-up'), 600); }
  ...
  /* ---------- Order detail (success + track detail) ---------- */
  ...
  const AUTO = { received: 8, prep: 12, grill: 16, almost: 10 }; // seconds per status
  function autoTick() {
    let changed = false;
    ...
    if (!changed) refreshLive(); // onChange handles the changed case
  }
  ...
  /* ---------- React to store changes (other tabs / staff) ---------- */
